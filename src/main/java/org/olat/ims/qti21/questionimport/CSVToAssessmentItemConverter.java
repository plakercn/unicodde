...
public class CSVToAssessmentItemConverter {
	...
	public void parse(String input) {
		List<String[]> lines = prepareParse(input);
		String str="";
	...
	public List<String[]> prepareParse(String input) {

		List<String[]> qtLines=new ArrayList<>();
		List<String[]> prepareLines = getLines(input);
		log.info("preparelines-size"+prepareLines.size());
		for(String[] line:prepareLines) {
            qtLines.addAll(prepareProcessLine(line));
		}
		return qtLines;
	}
    ...
    //将每一行（题目）转化为openolat要求的格式

	private List<String[]> prepareProcessLine(String[] parts) {
		String str="";
		...
		if(parts.length > 2) {
			marker = parts[2].toLowerCase();
			if (marker.equals("单选题")) {    //
				qtLines=processBranch(parts,"sc");
			} else if (marker.equals("多选题")) {
				qtLines=processBranch(parts,"mc");
			} else {
			}
		}
	...
	private  List<String[]>   processBranch(String parts[],String type) {

		int TITLE_LENGTH=30;

		List<String[]> qtLines = new ArrayList<>();
		if (parts.length < 2 || parts[1].equals("")) {
			log.info("Question content is blank!");
			return qtLines;
		}

		String value = "";
		String key = "";
		qtLines.add(new String[]{"type", type});

		value = parts[1];
	//	value = parts[1].substring(0, Math.min(parts[1].length(), TITLE_LENGTH)) + (parts[1].length() > TITLE_LENGTH ? "..." : "");
		qtLines.add(new String[]{"title", value});

		qtLines.add(new String[]{"question", ""});


		try {
			Float.parseFloat(parts[3]);
		} catch (Exception e) {
			log.info("Point must be a number. be setted zero");
			parts[3] = "0";
		}
		value = parts[3];
		qtLines.add(new String[]{"points", value});
		...
		switch (type) {
			case "sc": {
				...
				for (int i = 6; i < parts.length; i++) {
					if (parts[i] != null && !parts[i].equals("")) {
						...
						if (item.equals(parts[4].toUpperCase())) {  //asnswer
							key = parts[3];  //point
						}
						value = parts[i];
						qtLines.add(new String[]{key, value});
					}
				}
				break;
			}
			case "mc": {
				...
				float singlePoint = Float.parseFloat(parts[3])/ correctNum;
				for (int i = 6; i < parts.length; i++) {
					if (parts[i] != null && !parts[i].equals("")) {
						item = (char) (65 + i - 6) + "";
						if (parts[4].indexOf(item)>-1) {  //correct item
							key = df.format(singlePoint);  //point
						} else {
							key =  df.format(singlePoint*(-1) );  //point
						}

						qtLines.add(new String[]{key, parts[i]});
