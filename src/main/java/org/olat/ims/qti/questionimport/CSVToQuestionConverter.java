...
public class CSVToQuestionConverter {
	...
	public void parse(String input) {
		try {
			input = input.getBytes("UTF-8").toString();
		}catch (Exception e) {

		}
		String[] lines = input.split("\r\n");
        log.warn("allcontent_qt="+input);
	...
	private void processLine(String[] parts) {
		...
		switch (marker) {
			...
			case "beschreibung":
			case "description":
				processDescription(parts);
				break;
			case "frage":
			case "question":
				processQuestion(parts);
				break;
			case "punkte":
			case "points":
				processPoints(parts);
			...
			case "fachbereich":
			case "subject":
				processTaxonomyPath(parts);
				break;
			case "feedback correct answer":
				processFeedbackCorrectAnswer(parts);
				break;
			case "feedback wrong answer":
				processFeedbackWrongAnswer(parts);
				break;
			case "schlagworte":
			case "keywords":
				processKeywords(parts);
				break;
			case "abdeckung":
			case "coverage":
				processCoverage(parts);
				break;
			case "level":
				processLevel(parts);
				break;
			case "sprache":
			case "language":
				processLanguage(parts);
				break;
			case "durchschnittliche bearbeitungszeit":
			case "typical learning time":
				processTypicalLearningTime(parts);
				break;
			case "itemschwierigkeit":
			case "difficulty index":
				processDifficultyIndex(parts);
				break;
			case "standardabweichung itemschwierigkeit":
			case "standard deviation":
				processStandardDeviation(parts);
				break;
			case "trennsch\u00E4rfe":
			case "discrimination index":
				processDiscriminationIndex(parts);
				break;
			case "anzahl distraktoren":
			case "distractors":
				processDistractors(parts);
				break;
			case "editor":
				processEditor(parts);
				break;
			case "editor version":
				processEditorVersion(parts);
				break;
			case "lizenz":
			case "license":
				processLicense(parts);
				break;
			default:
				processChoice(parts);
		}
	}
	private void processLine_CN(String[] parts) {

		String marker = parts[2].toLowerCase();
		switch(marker) {
			case "单选题": {
                processTypeSC(parts);
                break;
			...
			case "多选题":processTypeMC(parts);break;
		//	default: processChoice(parts);
		}
	}
  private  void   processMore(String parts[]){
		...
		fields[1]=parts[1].substring(0,20)+"...";
		processTitle(fields); ;

		fields[0]="points";
		fields[1]=parts[3];
		processPoints(fields);
  }
	private void processTypeMC(String[] parts){
...
//	   case "level": processLevel(parts); break;
       ...
       int selectNum=0;
		...
		double singlePoint = Math.round (Float.parseFloat(parts[3])/selectNum);
		for(int i=6;i<10;i++) {
			if (parts[i] != null && !parts[i].equals("")) {
				if((char)(97+i-1)==(int)(parts[4].toCharArray()[0])) {  //asnswer
					fields[0] = df.format(singlePoint);  //point
				}else{
					fields[0] = df.format(singlePoint*(-1));  //point
				}
				fields[1]=parts[i];
				processChoice(fields);
			}
		}
	}

   private void processTypeSC(String[] parts){
