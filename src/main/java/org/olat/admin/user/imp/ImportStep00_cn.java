 ...
 * @author rhaag
 */
class ImportStep00_cn extends BasicStep {
	...
	public ImportStep00_cn(UserRequest ureq, boolean canCreateOLATPassword) {
		super(ureq);
	...
	private final class ImportStepForm00 extends StepFormBasicController {
		...
		protected boolean validateFormLogic(UserRequest ureq) {
			...
			Collection<String> languages = i18nModule.getEnabledLanguageKeys();
			 ...
			 textAreaElement.setErrorKey("error.login",new String[]{usageIdentifyer});
			...
			for (int i = 0; i < lines.length; i++) {
                ...
                //name column
				Identity ident = securityManager.findIdentityByName(login);  //mobile
				if (ident != null) {
		...
		private boolean updateUserProperties(Identity ud, User originalUser, String[] parts, int i, int columnId,
			...
			for (int j = i; j < userPropertyHandlers.size(); j++) {
				...
				logInfo(thisKey+"=="+thisValue,null);
